...
export class MoviesService {
  ...
  getTopRatedMovies(year: string): Observable<IMovie[]> {
    ...
    return this.http.get<any>(url, { params: params }).pipe(
      map((response) =>
        response.results.slice(0, 5).map((movie: IMovie) => ({
          id: movie.id,
          title: movie.title,
  ...
  search(
    ...
    this.http.get<IMovieResponse>(url, { params }).subscribe((result) => {
      this.movieDataSubject.next(this.formatResponse(result));
    });
